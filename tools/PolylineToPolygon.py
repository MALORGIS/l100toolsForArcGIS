...
class PolylineToPolygon(object):
  ...
  def __init__(self):
    self.label = _("Polyline To Polygon")
    self.description = _("Creates a Polygon feature class from specified Polyline features.")
  ...
  def getParameterInfo(self):
    ...
    param1.filter.list = ['Short', 'Long', 'Double', 'Single', 'Text', 'OID']
  ...
  def execute(self, parameters, messages):
    ...
    #create output
    arcpy.CreateFeatureclass_management(dirName, fcName, "POLYGON", None, inputHasM, inputHasZ, inDesc.spatialReference)
      ...
      with arcpy.da.SearchCursor(inFeatures, fieldList) as cursor:
        ...
        for row in sorted(cursor):
          ...
          if (isFirstTime):
            ...
            if (isinstance(row[-1] , arcpy.Polyline)):
              insertRow = (preGroup,) + (arcpy.Polygon(arcpy.Array(pntList)),)
        ...
        if (not groupField):
          insertRow = (arcpy.Polygon(arcpy.Array(pntList)),)
          inCursor.insertRow(insertRow)
        else:
          insertRow = (preGroup,) + (arcpy.Polygon(arcpy.Array(pntList)),)
          inCursor.insertRow(insertRow)
